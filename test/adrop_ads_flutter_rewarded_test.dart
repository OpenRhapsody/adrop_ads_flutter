...
main() {
  ...
  final listener = AdropRewardedListener(
    ...
    onAdEarnRewardHandler: (ad, type, amount) => handleEventMessage[ad] = 'Ad Earned type $type with amount $amount',
  );
  final MockAdropRewardedAd rewardedAd = MockAdropRewardedAd(unitId: unitId, listener: listener);
  ...
  group('load', () {
    test('load before initialized error', () async {
      ...
      expect(handleEventMessage[rewardedAd], 'Ad Failed to Receive with err: ${AdropErrorCode.initialize.name}');
    });

    test('load with invalid unitId error', () async {
      await Adrop.initialize(true);
      final nullRewardedAd = MockAdropRewardedAd(unitId: '', listener: listener);
      await nullRewardedAd.load();
      expect(handleEventMessage[nullRewardedAd], 'Ad Failed to Receive with err: ${AdropErrorCode.invalidUnit.name}');
      await nullRewardedAd.dispose();
    });

    test('load with non-exist unitId error', () async {
      await Adrop.initialize(true);
      final nonExistRewardedAd = MockAdropRewardedAd(unitId: nonExistUnitId, listener: listener);
      await nonExistRewardedAd.load();
      expect(handleEventMessage[nonExistRewardedAd], 'Ad Failed to Receive with err: ${AdropErrorCode.adNoFill.name}');
      await nonExistRewardedAd.dispose();
    });

    test('load with inactive unitId error', () async {
      await Adrop.initialize(true);
      final inactiveRewardedAd = MockAdropRewardedAd(unitId: inactiveUnitId, listener: listener);
      await inactiveRewardedAd.load();
      expect(handleEventMessage[inactiveRewardedAd], 'Ad Failed to Receive with err: ${AdropErrorCode.inactive.name}');
    ...
    test('load two rewarded ads', () async {
      await Adrop.initialize(true);
      final rewardedAd2 = MockAdropRewardedAd(unitId: unitId2, listener: listener);
    ...
    test('load duplicate error', () async {
      ...
      await rewardedAd.load();
      await rewardedAd.load();
      expect(handleEventMessage[rewardedAd], 'Ad Failed to Receive with err: ${AdropErrorCode.adLoadDuplicate.name}');
    });
    ...
    test('load while loading error', () async {
      ...
      rewardedAd.load();
      rewardedAd.load();
      expect(handleEventMessage[rewardedAd], 'Ad Failed to Receive with err: ${AdropErrorCode.adLoading.name}');
    });
  });
  ...
  group('show', () {
    test('show while loading error', () async {
      ...
      expect(handleEventMessage[rewardedAd], 'Ad Failed to Show with err: ${AdropErrorCode.adLoading.name}');
    });
    ...
    test('show after shown error', () async {
      ...
      await rewardedAd.show();
      await rewardedAd.show();
      expect(handleEventMessage[rewardedAd], 'Ad Failed to Show with err: ${AdropErrorCode.adShown.name}');
    });

    test('show not loaded ad error', () async {
      await Adrop.initialize(true);
      await rewardedAd.show();
      expect(handleEventMessage[rewardedAd], 'Ad Failed to Show with err: ${AdropErrorCode.adEmpty.name}');
    });
  ...
  test('click ad', () async {
    final adropEventObserverChannelName =
        AdropChannel.adropEventListenerChannelOf(AdType.rewarded, rewardedAd.requestId);
    final adropEventObserverChannel =
        adropEventObserverChannelName != null ? MethodChannel(adropEventObserverChannelName) : null;
  ...
  test('earn reward', () async {
    ...
    final adropEventObserverChannel =
        adropEventObserverChannelName != null ? MethodChannel(adropEventObserverChannelName) : null;
    await adropEventObserverChannel?.invokeMethod(AdropMethod.didHandleEarnReward, {'type': 1, 'amount': 100});
